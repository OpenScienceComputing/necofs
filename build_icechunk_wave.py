#!/usr/bin/env python
"""
Build Icechunk store for NECOFS WAVE forecast (single NetCDF3 file).

No subchunking, no Dask, no Coiled — one virtual dataset written to Icechunk.

Run:
    export $(grep -v '^#' ~/dotenv/gom3_forecast.env | xargs) && \
    conda run -n protocoast-notebook python build_icechunk_wave.py
"""
import os
...
REGION     = 'us-east-1'
SOURCE_KEY = 'WAVE/NECOFS_WAVE_FORECAST.nc'
...
parser   = HDFParser(drop_variables=['Itime', 'Itime2', 'Times', 'file_date', 'iint', 'nprocs',
                                     'siglay', 'siglev'])
...
snap = session.commit('NECOFS WAVE forecast — single virtual dataset, UGRID/CF-1.11')
